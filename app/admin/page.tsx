...
export default function AdminDashboard() {
  ...
  if (!stats) {
    return (
      <div style={{ padding: '48px', color: '#EF4444', fontSize: '14px' }}>데이터를 불러오지 못했습니다.</div>
    );
  }
  ...
  return (
    <div style={{ padding: '36px 40px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '16px' }}>
        ...
        <StatCard label="Pro 대기자" value={stats.proSignupsCount.toLocaleString()} color="#7C3AED" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px', marginBottom: '24px' }}>
        <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '20px' }}>
          <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: '#EEF2FF', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '32px', fontWeight: 600, color: '#4F46E5', lineHeight: 1 }}>{stats.mau.toLocaleString()}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
        ...
        <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '24px' }}>
          ...
          {[
            ...
            { label: 'Pro', value: stats.planBreakdown.pro, color: '#4F46E5' },
            { label: 'Team', value: stats.planBreakdown.team, color: '#7C3AED' },
          ].map(item => (
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: '8px', height: '100px' }}>
            {chartDays.map(([day, count]) => (
              <div key={day} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                ...
                <div style={{ width: '100%', height: `${(count / maxVal) * 72}px`, minHeight: '4px', background: 'linear-gradient(to top, #4F46E5, #7C3AED)', borderRadius: '4px 4px 0 0' }} />
