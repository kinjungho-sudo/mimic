...
export default function EditorPage() {
  ...
  // 실시간 협업 훅 — 워크스페이스 튜토리얼에서만 활성화
  const workspaceId = tutorial ? (tutorial as Tutorial & { workspace_id?: string | null }).workspace_id ?? null : null;
  const { broadcastStepChange, updatePresence } = useCollaboration({
    ...
    onRemoteStepChange: (stepId, patch) => {
      ...
      const editor = collaborators.find(c => c.activeStepId === stepId);
      if (editor) {
  ...
  // 활성 스텝 변경 시 Presence 업데이트 (협업 커서)
  useEffect(() => {
      ...
      {collabToast && (
        <div style={{
          ...
          <div style={{
            ...
            background: collabToast.color, display: 'grid', placeItems: 'center',
