  ...
  switch (id) {
    case 'intro':
      return (
        <div>
          ...
          {p('클릭 한 번 한 번이 자동으로 캡처되어 단계별 스크린샷 + 설명이 만들어집니다. 완성된 매뉴얼은 링크로 공유하거나, 실제 페이지 위에 Guide Me 오버레이로 단계별 안내를 제공합니다.')}
          {h3('MIMIC으로 할 수 있는 것')}
    ...
    case 'share':
      return (
        <div>
          {h2('4. 공유하기')}
          {p('완성된 매뉴얼을 링크로 공유하거나, Guide Me 오버레이로 실제 페이지에서 안내할 수 있습니다.')}
          {h3('링크 공유')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              '매뉴얼 상단 "공유" 버튼을 클릭합니다.',
          ...
          {h3('Guide Me 미리보기')}
          {p('매뉴얼에 원본 페이지 URL이 저장되어 있으면 "Guide Me" 버튼이 활성화됩니다. 클릭하면 실제 페이지 위에서 오버레이 가이드를 미리볼 수 있습니다.')}
        </div>
      );
    ...
    case 'guide-me':
      return (
        <div>
          {h2('Guide Me')}
          {p('Guide Me는 실제 웹페이지 위에 오버레이를 띄워 단계별로 안내하는 기능입니다. 사용자는 별도 화면 없이 실제 업무 페이지에서 바로 가이드를 받을 수 있습니다.')}
          {h3('사용 방법')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              '매뉴얼이 녹화된 원본 페이지 URL이 있어야 합니다.',
              '매뉴얼 상단 "Guide Me" 버튼을 클릭합니다.',
              '새 탭에서 원본 페이지가 열리면서 Guide Me 오버레이가 시작됩니다.',
          ...
          {h3('SDK로 직접 삽입')}
          {p('자체 서비스에 Guide Me를 삽입하려면 아래 스크립트를 페이지에 추가하세요.')}
            ...
            {[
              {
                ...
                color: '#F9FAFB',
                border: '#E5E7EB',
                features: ['일 3회 매뉴얼 생성', '기본 공유 링크', 'PDF 내보내기', 'Guide Me'],
              },
              {
                name: 'Pro',
                price: '문의',
      ...
      return (
        <div>
          ...
          {[
            {
              q: '매뉴얼 생성이 안 돼요.',
              a: '무료 플랜은 하루 3회까지 생성 가능합니다. 내일 다시 시도하거나 Pro 플랜으로 업그레이드하면 무제한으로 생성할 수 있습니다.',
            },
            {
              q: 'Guide Me가 버튼을 못 찾아요.',
              a: '원본 페이지의 UI가 변경됐을 수 있습니다. 에디터에서 "페이지 변경 감지 검사"를 실행하면 변경된 단계를 확인할 수 있습니다.',
            },
            {
              q: '공유 링크로 들어갔는데 아무것도 안 보여요.',
              a: '매뉴얼이 아직 초안(draft) 상태일 수 있습니다. "공유" 버튼 → "게시하고 공유"를 눌러 공개 상태로 전환해주세요.',
            },
            {
              q: 'PDF 내보내기가 느려요.',
              a: '스크린샷이 많은 매뉴얼은 생성에 시간이 걸릴 수 있습니다. 페이지를 닫지 말고 잠시 기다려주세요.',
            },
            {
              q: '팀원을 초대하려면 어떻게 하나요?',
              a: '팀 워크스페이스 설정에서 팀원 이메일로 초대 링크를 전송할 수 있습니다. Team 플랜이 필요합니다.',
            },
            {
              q: '확장 프로그램이 Chrome 외 브라우저에서도 동작하나요?',
              a: 'Chromium 기반 브라우저(Edge, Brave 등)에서도 동작합니다. Firefox, Safari는 현재 지원하지 않습니다.',
            },
          ].map(({ q, a }, i) => (
          ...
          <div style={{ padding: '16px 20px', background: '#F0F9FF', border: '1px solid #BAE6FD', borderRadius: '10px', marginTop: '8px' }}>
            ...
            <div style={{ fontSize: '13.5px', color: '#0c4a6e' }}>support@mimic.so로 문의해주세요. 빠르게 답변드리겠습니다.</div>
          </div>
        </div>
      );
