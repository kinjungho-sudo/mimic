...
// ── Tools ─────────────────────────────────────────────────
server.tool('list_tutorials', 'MIMIC에 저장된 매뉴얼 목록을 반환합니다. 실행할 워크플로우를 찾을 때 사용하세요.', {
...
server.tool('get_steps', '매뉴얼의 전체 단계 목록을 반환합니다. 실행 전 전체 흐름을 파악할 때 사용하세요.', {
    ...
    const { data: tut } = await supabase
        ...
        .single();
    if (!tut)
        return { content: [{ type: 'text', text: '매뉴얼을 찾을 수 없거나 접근 권한이 없습니다.' }] };
...
server.tool('get_step_detail', '특정 단계의 상세 실행 정보를 반환합니다. element_selector, 좌표, 스크린샷 URL 등을 포함합니다.', {
    ...
    const { data, error } = await supabase
        ...
        .single();
    if (error || !data)
        return { content: [{ type: 'text', text: 'Step을 찾을 수 없거나 접근 권한이 없습니다.' }] };
    const detail = {
        id: data.id,
...
// ── Auto-Run Tools ────────────────────────────────────────
server.tool('start_execution', '매뉴얼 자동 실행 세션을 시작합니다. 실행 전 pre-flight 정보(실행 가능 스텝 수, 위험 스텝)를 반환합니다.', {
...
server.tool('save_step_result', '스텝 실행 결과를 저장합니다. 각 스텝 실행 후 반드시 호출하세요.', {
...
server.tool('finish_execution', '매뉴얼 자동 실행 세션을 종료합니다. 모든 스텝 실행 후 반드시 호출하세요.', {
