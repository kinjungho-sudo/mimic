...
> **⚠️ 2026-06-15 갱신**: 이 PRD의 핵심(실제 페이지 오버레이 가이드)은 **구현 완료**됨
> (`mimic_recorder/guide-engine.js` — selector→좌표 폴백, 타깃 클릭 자동 진행, 진행도/완료, 자체검증 14/14, 실사이트 라이브 검증 대기).
> 또한 제품 방향이 확장됨 — Guide Me는 이제 **① 연습 모드(웹·스샷, 확장 불필요)** + **② 실전 모드(확장, 실제 페이지)** 듀얼로,
> "인터랙티브 매뉴얼"이라는 제품 라인으로 정식 편입됨. **최신 설계는 `MIMIC_PRODUCT_DESIGN_v2.md` Section 10 참조.**
> 본 PRD는 실전(확장) 모드의 원본 명세로 유지하되, 데이터 모델/생성흐름/요금제는 Section 10이 우선한다.
...
| 어노테이션 (박스 하이라이트) | ✅ 완료 |
| 어노테이션 (코멘트 자동 입력) | ✅ 완료 |
| Guide Me 실전 엔진(오버레이·자동진행) | ✅ 구현(라이브 검증 대기) — guide-engine.js |
| Guide Me 연습 모드(웹·스샷) | 🔲 신규 예정 (Section 10) |
