...
git stash store -m "PC migration: guide toc" origin/backup/pc-move-20260713-stash-guide-toc-sanitized
...
과거 로컬 히스토리에는 `mimic_recorder/.env`가 추적된 적이 있다. GitHub에는 해당 파일을 제거한 `backup/pc-move-20260712-pre-reword`와 `backup/pc-move-20260713-stash-guide-toc-sanitized` 정제 브랜치가 보존되어 있다. 이전의 `backup/pc-move-20260712-stash-guide-toc` 브랜치는 사용하지 않는다. 별도 이전한 키와 토큰은 새 장비 복원 후 교체 여부를 확인한다.
