...
- 영역: **editor / api / recorder / landing** (각자 `feat/*` 브랜치).
...
### 6-2. 녹화→캡처 흐름 (확장 필요)
Chrome 확장(`mimic_recorder`)은 **녹화 캡처 파이프라인에서만** 필요.

**설치(언패킹):** `chrome://extensions` → 우상단 **개발자 모드 ON** → **압축해제된 확장 프로그램 로드** → `mimic_recorder` 폴더 선택.

> ⚠️ **현재 확장은 운영(project1)에 하드코딩**돼 있다 (`background.js`의 `SUPABASE_URL`/`WEBAPP_ORIGIN`, 업로드 버킷 `naviaction`). 그대로 녹화하면 **운영 DB·운영 스토리지에 기록**된다 → dev 테스트 불가 + 운영 오염.
>
> **dev에서 녹화 흐름을 테스트하려면** 확장에 dev 타깃 전환(dev Supabase + localhost 앱)이 필요하다. 미구현 상태 → 필요 시 "확장 dev 모드" 작업으로 처리(아래 TODO).
...
| 확장으로 운영에 녹화 후 "테스트"라 착각 | 확장 dev 모드(미구현) 후 테스트 |

---

## 8. 미해결 TODO
- **확장 dev 모드**: `mimic_recorder`가 dev DB(dskphg…)+localhost 앱을 타깃하도록 전환 스위치 추가. dev 프로젝트에 `naviaction` 버킷 추가 필요. (구현 전까진 녹화 흐름 dev 테스트 불가)
