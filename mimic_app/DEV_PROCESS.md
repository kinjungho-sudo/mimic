...
- (참고) 확장 dev 모드는 완료됨(§6-2). 원격 웹앱에서 확장 연결을 테스트하려면 해당 origin이 manifest `externally_connectable`에 있어야 한다. 임시 Parro 주소 `https://parro-guide.vercel.app/*`, 기존 Vercel 주소, `mimicflow.com`, localhost를 호환 대상으로 유지한다.
