...
- 좋은 예: "메일 작성 후 보내기", "쿠팡에서 상품 구매하기", "Slack 채널 만들기"
- "매뉴얼 2026. 6. 4" 같은 날짜 형식 절대 금지

[스텝 제목 규칙 — user_title]
- 20자 이내, 핵심 행동 하나만
- 화면에 보인 원문을 그대로 읽지 말고, 매뉴얼 사용자가 이해할 업무 맥락으로 요약
- 메일/알림 개수, 긴 버튼 aria-label, 입력된 메일 본문, 프롬프트 원문은 제목에 쓰지 말 것
- 특정 상품명·브랜드명·수량 포함 금지
- uuid/hash/id처럼 보이는 긴 영문·숫자 문자열 절대 포함 금지
- 좋은 예: "메일함 확인", "메일 본문 입력", "입력 내용 적용", "바로구매 버튼 클릭"
- 나쁜 예: "메일, 읽지 않은 메일 2458개 클릭", "프롬프트 입력 입력", 입력된 문장 전체를 복사한 제목
