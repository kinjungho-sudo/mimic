  ...
  switch (id) {
    ...
    case 'install':
      return (
        <div>
          {h2('1. 확장 프로그램 설치')}
          {p('MIMIC Recorder는 Chrome 브라우저에서 동작하는 확장 프로그램입니다. 설치 후 MIMIC 대시보드에서 바로 녹화를 시작할 수 있습니다.')}
          {h3('설치 방법')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              'Chrome 웹 스토어에서 "MIMIC Recorder"를 검색해 설치합니다.',
              'MIMIC 대시보드에서 "새 매뉴얼" 버튼을 클릭합니다.',
    ...
    case 'record':
      return (
        <div>
          {h2('2. 화면 녹화하기')}
          {p('녹화는 MIMIC 대시보드에서 시작합니다. 평소처럼 업무를 진행하면 클릭할 때마다 자동으로 캡처됩니다.')}
          {h3('녹화 시작')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              '대시보드 우측 상단 "새 매뉴얼" 버튼을 클릭합니다.',
          ...
          {h3('녹화 완료 후')}
          {p('녹화가 완료되면 AI가 자동으로 각 단계의 제목과 설명을 생성합니다. 대시보드 "매뉴얼" 탭에서 생성된 매뉴얼을 확인할 수 있습니다.')}
          {h3('직접 만들기')}
          {p('녹화 없이 직접 만들 수도 있습니다. "새 매뉴얼" 드롭다운에서 "빈 매뉴얼 만들기"를 선택하면 빈 에디터가 열립니다.')}
        </div>
      );
    ...
    case 'dashboard':
      return (
        <div>
          {h2('대시보드')}
          {p('대시보드는 모든 콘텐츠를 관리하는 메인 화면입니다.')}
          {img('/help/dashboard.png', '대시보드 — 사이드바·폴더·콘텐츠 목록')}
          {h3('콘텐츠 유형 탭')}
          {p('상단에 4개의 탭으로 콘텐츠 유형을 전환할 수 있습니다.')}
          <ul style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              '매뉴얼 — 녹화로 만든 단계별 스크린샷 가이드',
              '실습하기 — 캡처 화면 위에서 직접 클릭해보며 익히는 인터랙티브 연습',
              '플레이북 — 여러 매뉴얼을 하나로 엮은 통합 문서',
              'Live Guide — 실제 페이지 위에서 실행할 매뉴얼 목록',
            ].map(li)}
          </ul>
          {h3('새 콘텐츠 만들기')}
          {p('"새 매뉴얼" 버튼을 클릭하면 드롭다운이 열립니다. 녹화로 만들거나 빈 매뉴얼·플레이북을 선택할 수 있습니다.')}
    ...
    case 'practice':
      return (
        <div>
          {h2('실습하기')}
          {p('실습하기는 녹화된 화면 위에서 직접 클릭해보며 단계를 익히는 인터랙티브 연습 모드입니다. 영상을 보거나 문서를 읽는 대신, 실제 캡처 화면 위의 핫스팟을 클릭하며 진행합니다.')}
          {h3('대시보드에서 보기')}
          {p('대시보드 상단 "실습하기" 탭을 클릭하면 실습 가능한 매뉴얼 목록이 표시됩니다. 매뉴얼이 녹화되어 공개 상태이면 자동으로 실습하기가 생성됩니다.')}
          {h3('실습하기 공유')}
          {p('매뉴얼 상세 페이지에서 "실습하기" 링크를 복사해 팀원에게 공유하세요. 받는 사람은 별도 설치 없이 브라우저에서 바로 실습할 수 있습니다.')}
          {h3('실습 진행 방법')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
          ...
          {h3('실습하기 vs 라이브 가이드 차이')}
          <ul style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              '실습하기 — 캡처된 스크린샷 위에서 연습 (실제 사이트 미접속)',
    ...
    case 'playbook':
      return (
        <div>
          ...
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              '대시보드 "새 매뉴얼" 드롭다운에서 "새 플레이북(통합 문서)"를 선택합니다.',
              '또는 대시보드 "플레이북" 탭에서 "새 플레이북 만들기"를 클릭합니다.',
      ...
      return (
        <div>
          ...
          {[
            ...
            {
              q: '실습하기와 라이브 가이드의 차이가 뭔가요?',
              a: '실습하기는 캡처된 스크린샷 위에서 연습하는 모드로, 실제 사이트에 접속하지 않습니다. 라이브 가이드는 MIMIC 익스텐션이 실제 웹사이트 위에 오버레이를 띄워 직접 안내합니다. 학습용에는 실습하기, 실제 업무 수행 안내에는 라이브 가이드를 사용하세요.',
            },
            {
              q: '플레이북이 뭔가요?',
              a: '여러 매뉴얼과 텍스트를 하나의 통합 문서로 엮는 기능입니다. 대시보드 "새 매뉴얼" 드롭다운에서 "새 플레이북(통합 문서)"을 선택하거나, "플레이북" 탭에서 만들 수 있습니다. 슬래시(/) 명령으로 제목, 목록, 코드블록, 가이드 임베드 등의 블록을 추가할 수 있습니다.',
            },
            {
