  ...
  switch (id) {
    case 'intro':
      return (
        <div>
          ...
          {p('클릭 한 번 한 번이 자동으로 캡처되어 단계별 스크린샷과 설명이 만들어집니다. 완성된 매뉴얼은 링크와 문서로 공유하고, 필요하면 연습 가이드나 확장 프로그램 기반 라이브 가이드 Beta로 실행을 안내할 수 있습니다.')}
          {h3('MIMIC으로 할 수 있는 것')}
    ...
    case 'dashboard':
      return (
        <div>
          ...
          {p('연습 가이드와 라이브 가이드 Beta는 매뉴얼의 스튜디오에서 만들고 관리합니다.')}
          {h3('새 콘텐츠 만들기')}
    ...
    case 'practice':
      return (
        <div>
          {h2('연습 가이드')}
          {p('연습 가이드는 녹화된 화면 위에서 직접 클릭해보며 단계를 익히는 인터랙티브 연습입니다. 문서로 정리된 절차를 캡처 화면 위 행동으로 이어주며, 받는 사람은 별도 설치 없이 브라우저에서 연습할 수 있습니다.')}
          {h3('연습 가이드 만들기')}
          {p('매뉴얼의 스튜디오에서 연습 가이드를 생성·편집·공유합니다. 녹화된 매뉴얼이면 스튜디오에서 연습 가이드를 만들 수 있습니다.')}
          {h3('연습 가이드 공유')}
          {p('스튜디오에서 연습 가이드 링크를 복사해 팀원에게 공유하세요. 받는 사람은 별도 설치 없이 브라우저에서 바로 따라할 수 있습니다.')}
          {h3('연습 진행 방법')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
    ...
    case 'share-link':
      return (
        <div>
          ...
          {img('/help/share-player.png', '공유 플레이어 — 슬라이드·웹 문서·연습 가이드로 볼 수 있어요')}
          {h3('공유 링크 생성')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              '매뉴얼 상단 "공유" 버튼 클릭',
            ...
            {[
              {
                ...
                color: '#F9FAFB',
                border: '#E5E7EB',
                features: ['일 3회 매뉴얼 생성', '기본 공유 링크', 'PDF 내보내기', '연습 가이드 확인'],
              },
              {
                name: 'Pro',
                price: '문의',
                color: '#EEF2FF',
                border: '#a5b4fc',
                features: ['매뉴얼 생성 한도 확대', 'PDF/PPTX/Word 내보내기', '비밀번호 보호', '연습 가이드 + 라이브 가이드 Beta'],
                highlight: true,
              },
              {
                name: 'Team',
      ...
      return (
        <div>
          ...
          {[
            {
              q: 'MIMIC이 뭔가요?',
              a: 'Chrome 확장 프로그램으로 업무 화면을 녹화해 SOP와 인터랙티브 매뉴얼을 자동 생성하는 서비스입니다. 클릭 동작이 자동 캡처되어 단계별 스크린샷과 설명이 만들어지고, 링크·문서·연습 가이드로 공유할 수 있습니다. 조건이 맞으면 라이브 가이드 Beta로 실제 페이지 위 안내도 사용할 수 있습니다.',
            },
            {
            ...
            {
              q: '연습 가이드와 라이브 가이드 Beta의 차이가 뭔가요?',
              a: '연습 가이드는 캡처된 스크린샷 위에서 연습하는 모드로, 실제 사이트에 접속하지 않습니다. 라이브 가이드 Beta는 원본 URL과 확장 프로그램 조건이 맞을 때 실제 웹사이트 위에 오버레이를 띄워 안내합니다. 먼저 연습 가이드로 안정적으로 공유하고, 실제 업무 수행 안내가 필요할 때 라이브 가이드 Beta를 사용하세요.',
            },
            {
              q: '플레이북이 뭔가요?',
            ...
            },
            {
              ...
              a: '무료는 일 3회 매뉴얼 생성, 기본 공유, PDF 내보내기를 지원합니다. Pro는 생성 한도 확대, PDF/PPTX/Word 내보내기, 비밀번호 보호, 연습 가이드와 라이브 가이드 Beta를 포함할 예정입니다. 가격 문의는 support@mimic.so로 연락해주세요.',
            },
            {
              q: '문의는 어떻게 하나요?',
