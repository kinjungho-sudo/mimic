  ...
  switch (id) {
    case 'intro':
      return (
        <div>
          {h2('포리가란?')}
          {p('포리는 Chrome 확장 프로그램으로 업무 화면을 녹화해 SOP와 인터랙티브 매뉴얼을 자동 생성하고, 필요한 순간에는 화면 위 가이드로 실행까지 연결하는 서비스입니다.')}
          {img('/help/product-overview.png', '포리 — SOP 제작부터 화면 위 실행 안내까지')}
    ...
    case 'install':
      return (
        <div>
          {h2('1. 확장 프로그램 설치')}
          {p('포리 Recorder는 Chrome 브라우저에서 동작하는 확장 프로그램입니다. 설치 후 포리 홈 화면에서 바로 녹화를 시작할 수 있습니다.')}
          {h3('설치 방법')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              'Chrome 웹 스토어에서 "포리 Recorder"를 검색해 설치합니다.',
              '포리 홈 화면에서 "새로 만들기" 버튼을 클릭합니다.',
              '"새 매뉴얼(녹화)"를 선택하면 녹화 모달이 열립니다. 여기서 확장 연동이 자동으로 처리됩니다.',
            ].map(li)}
          </ol>
          {h3('확장을 찾을 수 없을 때')}
          {p('녹화 모달에서 "포리 Recorder 설치하기" 버튼이 나타나면 스토어로 이동해 설치하세요. 설치 후 "설치 완료 — 다시 시도" 버튼을 누르면 바로 연결됩니다.')}
          {h3('지원 브라우저')}
    ...
    case 'record':
      return (
        <div>
          {h2('2. 화면 녹화하기')}
          {p('녹화는 포리 홈 화면에서 시작합니다. 평소처럼 업무를 진행하면 클릭할 때마다 자동으로 캡처됩니다.')}
          {h3('녹화 시작')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
              ...
              '"녹화 시작" 버튼을 누르면 해당 탭이 활성화되고 우측에 포리 Recorder 사이드 패널이 열립니다.',
    ...
    case 'guide-me':
      return (
        <div>
          {h2('라이브 가이드 Beta')}
          {p('라이브 가이드 Beta는 실제 웹페이지 위에 오버레이를 띄워 단계별로 안내하는 기능입니다. 현재는 원본 URL이 저장된 매뉴얼과 포리 Recorder 확장 프로그램이 필요하며, 페이지 구조가 바뀌면 일부 단계가 맞지 않을 수 있습니다.')}
          {img('/help/live-guide.jpg', '라이브 가이드 Beta — 실제 페이지 위 AI 말풍선·핫스팟')}
          {h3('사용 조건')}
          {p('포리 Recorder 확장 프로그램이 설치되어 있고, 녹화 시 원본 페이지 URL이 저장되어 있어야 합니다. 무료 플랜에서는 사용량 제한이 적용될 수 있습니다.')}
          {h3('실행 방법 (받는 사람)')}
          <ol style={{ paddingLeft: '20px', margin: '0 0 16px' }}>
            {[
        ...
        <div>
          ...
          {[
            ...
            },
            {
              ...
              a: 'Chrome 웹 스토어에서 "포리 Recorder"를 검색해 설치하세요. Edge, Brave 등 Chromium 기반 브라우저도 지원합니다. Firefox, Safari는 현재 지원하지 않습니다.',
            },
            {
              q: '매뉴얼 생성이 안 돼요.',
