...
function renderAnnotations(annotations) {
  ...
  annotations.forEach((annotation) => {
    ...
    if (annotation.type === 'text') {
      ...
      Object.assign(element.style, {
        left: `${x1}%`, top: `${y1}%`, width: `${Math.max(1, Math.sqrt(dx * dx + dy * dy))}%`,
        height: `${stroke}px`, borderRadius: `${stroke}px`, background: color,
        transformOrigin: '0 50%', transform: `rotate(${Math.atan2(dy, dx) * 180 / Math.PI}deg)`,
      });
      if (annotation.type === 'arrow') {
        const head = document.createElement('span');
        Object.assign(head.style, {
          position: 'absolute', right: '-2px', top: '50%', width: '9px', height: '9px',
