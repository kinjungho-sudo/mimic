...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'CAPTURE_SCREENSHOT') {
    ...
    (async () => {
      ...
      // 정확히 일치하는 선캡처가 없으면 클릭 전 DOM rect를 클릭 후 화면에 덮어쓰지 않는다.
      // 같은 페이지는 작은 클릭 포인트로, 페이지가 바뀌었으면 자동 annotation 없이 저장한다.
      if (stepData.usePrecapture && !capturedRaw) {
        const sourcePage = comparableFrameUrl(stepData.url);
        const currentPage = comparableFrameUrl(tab.url);
        const samePage = !!sourcePage && !!currentPage && sourcePage === currentPage;
        stepData = {
          ...stepData,
          elementRect: null,
